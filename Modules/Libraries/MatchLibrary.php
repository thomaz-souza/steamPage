...
*/
	...
	class MatchLibrary extends _Core 
	{
		...
		const EMAIL = '/^[A-z0-9-_.]+@[A-z0-9_-]+\.[A-z0-9-_.]+$/';

		const MOBILE = '/^\([0-9]{2}\) [9]{0,1}[\s]{0,1}[0-9]{4}\-[0-9]{3,4}$/';
		...
		const PERSONAL_NAME =  '/^[A-zÀ-ÖØ-öø-ÿ\s\.\']+$/';

		const PERSONAL_COMPLETE_NAME =  '/^[A-zÀ-ÖØ-öø-ÿ\.\']+\s{1,}[A-zÀ-ÖØ-öø-ÿ\s\.\']+$/';

		const ANY =  '/(.*)/';

		const DATE = '/^[0-3][0-9]\/[0-1][0-9]\/[1-9][0-9]{3}$/';

		const DATE_ISO = '/^[0-9]{4}\-[0-1]{1}[0-9]{1}\-[0-3]{1}[0-9]{1}$/';
